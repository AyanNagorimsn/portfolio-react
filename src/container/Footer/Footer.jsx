...
const Footer = () => {
    ...
    const sendEmail = (e) => {
        e.preventDefault();
        emailjs.sendForm("service_z2ha80b", "template_tmob5mj", form.current, "aJLGTGV4AsVti6LM_").then(
            (result) => {
                console.log(result);
            },
            (error) => {
    ...
    return (
        <div id="contact" style={{ paddingTop: "6rem" }}>
            ...
            <div className="app__footer-cards">
                {["mobile", "email"].map((el) => {
                    return (
                        <div key={el} className="app__footer-card" data-tooltip-content="Tap to copy" data-tooltip-id="tooltip" onClick={(e) => copyText(e)}>
            ...
            <p className="copyright">Created by Ayan Nagori with 💖 @{new Date().getFullYear()}</p>
        </div>
    );
};
